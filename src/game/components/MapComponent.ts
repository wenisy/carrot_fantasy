...
export class MapComponent extends Component {
        ...
        for (let y = 0; y < this.height; y++) {
            ...
            for (let x = 0; x < this.width; x++) {
                ...
                tiles[y][x] = {
                    ...
                    walkable: isOnPath // 路径是可行走的，草地可以放置塔
                }
            }
        }
    ...
    private isOnPath(x: number, y: number): boolean {
        ...
        for (let i = 0; i < this.waypoints.length - 1; i++) {
            ...
            // 简化路径检测 - 检查是否在路径线上（允许1格的误差）
            if (this.isPointOnLine(x, y, start.x, start.y, end.x, end.y, 1.0)) {
                return true
            }
        }
        ...
        for (const waypoint of this.waypoints) {
            if (Math.abs(x - waypoint.x) <= 0.5 && Math.abs(y - waypoint.y) <= 0.5) {
                return true
            }
        }
